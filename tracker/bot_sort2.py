...
class BoTSORT2(object):
    ...
    def update(self, output_results, img):
        ...
        if len(dets) > 0:
            ...
            '''Detections'''
            if self.args.with_reid:
                detections = [STrack2(STrack2.tlbr_to_tlwh(tlbr), s, f) for
        ...
        if len(dets_second) > 0:
            '''Detections'''
            detections_second = [STrack2(STrack2.tlbr_to_tlwh(tlbr), s) for
                                 (tlbr, s) in zip(dets_second, scores_second)]
        else:
            detections_second = []

        r_tracked_stracks = [strack_pool[i] for i in u_track if strack_pool[i].state == TrackState2.Tracked]
