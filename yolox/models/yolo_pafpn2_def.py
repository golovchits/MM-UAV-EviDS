...
class Fusion(nn.Module): # ASPP 加权融合
    ...
    def forward(self, x_rgb, x_ir):
        ...
        if Vis:
            save_with_sequence(x_rgb_offset, "x_rgb_offset", "saved_npy")
            save_with_sequence(x_ir_offset, "x_ir_offset", "saved_npy")

        # 自适应加权融合
        x_rgb_output = self.AdaptiveFusion_for_rgb(x_rgb, x_ir_offset) + x_rgb
        x_ir_output = self.AdaptiveFusion_for_ir(x_ir, x_rgb_offset) + x_ir
