...
class YOLOXHead(nn.Module):
    def __init__(
        ...
        # Temperature scaling (default 1.0 = no effect)
        self.T = 1.0
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        y_shifts = []
        expanded_strides = []
        # Reset logit storage for each forward pass
        self._last_obj_logits = None
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                ...
                # Store raw logits per FPN level for temperature scaling calibration
                if not hasattr(self, '_last_obj_logits') or self._last_obj_logits is None:
                    self._last_obj_logits = []
                    self._last_cls_logits = []
                # Apply temperature scaling before sigmoid
                output = torch.cat(
                    [reg_output,
                     (obj_output / self.T).sigmoid(),
                     (cls_output / self.T).sigmoid()], 1
                )
                self._last_obj_logits.append(obj_output)
                self._last_cls_logits.append(cls_output)
