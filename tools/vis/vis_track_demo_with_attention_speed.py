...
output_video_path = os.path.join(output_video_dir, 'multi-qifei-speed.mp4')
...
for frame_file in frame_files:
    ...
    if frame_id in tracking_data:
        ...
        for track in filtered_tracks:
            ...
            if len(track_history[track_id]) > 5:  # 保留最近5帧的位置
            ...
            if len(track_history[track_id]) >= 2:
                ...
                speed_x = current_x - prev_x
                speed_y = current_y - prev_y
                speed = np.sqrt(speed_x**2 + speed_y**2)  # 计算速度大小
            ...
            approaching_text = "Up"
            font_scale = max(w / 90.0, 0.2)  # 根据框宽度调整字体大小
...
print(f"Tracking visualization video with speed information has been saved to {output_video_path}")