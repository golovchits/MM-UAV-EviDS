...
# 跟踪结果文件路径
tracking_results_path = '/mnt/sda/Disk_B/jinjie/Tracking/MOTs/BoT-SORT-2stream/YOLOX_outputs/track_demos/track_results/zaiwu-yuanli.txt'

# 图片路径
frames_dir = '/mnt/sda/Disk_B/jinjie/Tracking/MOTs/BoT-SORT-2stream/videos-for-demo/zaiwu-yuanli/'

# 输出视频路径
output_video_dir = '/mnt/sda/Disk_B/jinjie/Tracking/MOTs/BoT-SORT-2stream/videos-for-demo/zaiwu-yuanli/results/'
...
output_video_path = os.path.join(output_video_dir, 'zaiwu-yuanli-behavior.mp4')
out = cv2.VideoWriter(output_video_path, fourcc, 30.0, (width, height))

# 设置帧范围
start_frame = 0  # 从第70帧开始
end_frame = 150  # 到第140帧结束
...
# predefined_behavior = {
#     "Static": [0, 30],       # 0-30帧静止
#     "Move Away": [31, 60],   # 31-60帧远离
#     "Up": [61, 90],          # 61-90帧上升
#     "Down": [91, 120],       # 91-120帧下降
#     "Orbiting": [121, 150],  # 121-150帧环绕
#     "Tracking": [151, 160],  # 151-180帧跟踪
#     "Moving": [161, end_frame]  # 181帧到最后为移动
# }
# 手动设置的行为列表
# predefined_behavior = {
#     "Up": [70, end_frame]  # 181帧到最后为移动
# }
# 手动设置的行为列表
predefined_behavior = {
    "Hover": [0, 30],  # 0-20帧静止
    ...
    if frame_id in tracking_data:
        ...
        for track in filtered_tracks:
            ...
            if len(track_history[track_id]['positions']) >= 2:
                ...
                speed = np.mean(track_history[track_id]['speeds'])*2  # 使用最近几帧的平均速度

            # 手动设置行为信息
            behavior = "Moving"  # 默认行为
            for key, value in predefined_behavior.items():
                if value[0] <= frame_id <= value[1]:
                    behavior = key
                    break
            ...
            if behavior == "Approaching":
                ...
                text_color_behavior = (255, 165, 0)  # 橙色
            elif behavior == "Down":
                text_color_behavior = (0, 165, 255)  # 青色
            elif behavior == "Orbiting":
                text_color_behavior = (255, 0, 255)  # 紫色
            elif behavior == "Tracking":
                text_color_behavior = (0, 255, 255)  # 黄色
            else:
